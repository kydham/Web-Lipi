...
function melos_thinkup_child_frontscripts() {
	...
	wp_enqueue_style( 'melos-thinkup-style-grid', get_stylesheet_directory_uri() . '/style.css', array( 'thinkup-style' ), wp_get_theme()->get('Version') );
}
...
function melos_thinkup_updateoption_child_settings() {
	...
	if( ! empty( $options_free ) ) {
		...
		if ( $options_child_settings != 1 ) {
			...
			$options_free['thinkup_styles_skin']       = 'grid';
